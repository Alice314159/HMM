...
def generate_comprehensive_analysis(pipeline, train_df, test_df, train_index, test_index,
    ...
    train_analyzer.generate_emission_matrix_report()
...
def train_model_improved(config_path: str = "config.yaml"):
    ...
    try:
        ...
        # 打印验证报告
        logger.info("\n" + "="*60)
        logger.info("数据处理一致性验证报告")
        logger.info("="*60)
        logger.info(f"✓ 原始训练数据形状: {train_df_original.shape}")
        logger.info(f"✓ 特征数据形状: {featured_train_df.shape}")
        logger.info(f"✓ 处理后数据形状: {X_train_processed.shape}")
        logger.info(f"✓ 完整训练数据形状: {complete_train_df.shape}")
        logger.info(f"✓ 特征维度匹配: {validation_result['shape_match']}")
        logger.info(f"✓ 特征名称一致: {validation_result['feature_names_match']}")
        logger.info("="*60)
...
if __name__ == "__main__":
    config_file = r"E:\Vesper\HMM0\config.yaml"
